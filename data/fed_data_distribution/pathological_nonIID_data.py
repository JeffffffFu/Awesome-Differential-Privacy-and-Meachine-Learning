...
def pathological_split_noniid(train_data, n_clients, alpha, seed,q):
    ...
    for ii in range(n_clients):  # 循环客户端
        ...
        # 这样就组装好了一个客户端的ind，然后用这个ind分别取data和标签数据
        # 上面把iid处理好了，我们现在要将数据拉到字段中

        # sample_name="sample"+str(ii)
        ...
        # train_labels.append(np.unique(data_info.numpy()))  # 把每个客户端的标签加到train_labels并去重，这样我们就知道每个客户端有什么标签
